  ...
  return (
    <main className="min-h-screen bg-[#f4f6fb] text-[#1f2937] relative">
      ...
      <div className="max-w-full lg:max-w-[80%] mx-auto px-4 sm:px-6 py-12 sm:py-16">
        <h1 className="text-2xl sm:text-3xl font-semibold text-[#111827]">{page.title}</h1>
        {paragraphs.map((paragraph, index) => (
          <p key={index} className={index === 0 ? 'mt-4 text-base text-[#1f2937]' : 'mt-3 text-base text-[#1f2937]'}>
            {paragraph}
          </p>
        ))}
        <div className="mt-6 rounded-2xl border border-[#e5e7eb] bg-white p-5 sm:p-6 shadow-sm">
